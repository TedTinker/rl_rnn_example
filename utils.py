...
class RecurrentReplayBuffer:
    ...
    def __init__(
        ...
        o_dim = 2,
        a_dim = 1,
        max_episode_len = 1000,  # this will also serve as num_bptt
...
def plot_durations(episode_durations, episode_num):
    """Plot the durations of episodes (light blue) and their rolling average (red) over 100 episodes."""
    durations_t = torch.tensor(episode_durations, dtype=torch.float)
    
    plt.title('Episode Durations')
    plt.xlabel('Episode')
    plt.ylabel('Duration')
    plt.plot(durations_t.numpy(), color = (0, 0, 1, .2))
    ...
    plt.savefig("plots/{}/durations.png".format(episode_num))
    plt.show()
    plt.close()
